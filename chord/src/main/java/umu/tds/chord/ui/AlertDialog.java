	...
	private void registerSongStatusListener() {
		Controller.INSTANCE.registerSongStatusListener(new SongStatusListener() {
			...
			public void onSongLoad(SongStatusEvent e) {
				if (e.isFailed())
					JOptionPane.showMessageDialog(null, errorLoadDesc, errorLoad, JOptionPane.ERROR_MESSAGE);
			}

			@Override
			public void onSongDelete(SongStatusEvent e) {
				if (e.isFailed())
					JOptionPane.showMessageDialog(null, errorDeleteDesc, errorDelete, JOptionPane.ERROR_MESSAGE);
			}
		});
	}
}
