...
public enum TDSPlaylistDAO implements DAO<Playlist.Internal> {
	...
	private enum Properties {
		DESCRIPTION, NAME, PLAYLIST_ENTITY_TYPE, SONGS,
	}
	 ...
	 *         estar registrado y ser de tipo playlist.
	 */
	@Override
	public Optional<Playlist.Internal> recover(int id) {
		...
		name = persistence.recuperarPropiedadEntidad(ePlaylist, Properties.NAME.name());
		description = persistence.recuperarPropiedadEntidad(ePlaylist, Properties.DESCRIPTION.name());
	...
	public boolean register(Playlist.Internal p) {
		...
		// Propiedades.
		ePlaylist.setPropiedades(Arrays.asList(new Propiedad(Properties.NAME.name(), p.getName()),
